...
**Problem**: `gitManager` (gRPC) writes to ArangoDB via `entitygraph.DataManager`;
`GitHTTPHandler` (Smart HTTP) reads from the filesystem via go-git. Data is never
synchronised — a `git clone` against a repo created via gRPC fails because no
`.git/` directory exists on disk.

**Design decision**: The `storage.Storer` implementation delegates **down** to
`entitygraph.DataManager` — not to raw ArangoDB collections. This keeps a single
unified storage layer across all CodeVald services.

See: [mvp-details/arangodb-storer.md](mvp-details/arangodb-storer.md) · Gap resolutions: [architecture-storer-gaps.md](../2-SoftwareDesignAndArchitecture/architecture-storer-gaps.md)

#### GIT-015a — Schema additions (`schema.go`)

| Task | Status | Depends On |
|------|--------|------------|
| GIT-015a: Add `sha` to `Branch`, `head_ref` to `Repository`, new `GitInternalState` TypeDefinition | ✅ Done | ~~GIT-010~~ ✅ |

**Scope**:
- `schema.go` — add `sha` (string) to `Branch` TypeDefinition: the target commit hash; updated by `SetReference` so Smart HTTP can resolve refs in one read.
- `schema.go` — add `head_ref` (string) to `Repository` TypeDefinition: the symbolic HEAD target (e.g. `"refs/heads/main"`); written on `InitRepo`.
- `schema.go` — add `GitInternalState` TypeDefinition; `StorageCollection: "git_internal"`; `UniqueKey: ["state_type"]`; properties: `state_type` (`"config"` | `"index"` | `"shallow"`), `data` (base64 string). Consumed only by the storer.

#### GIT-015b — Real git SHA + `entries` property + `advanceBranchHead` sha write

| Task | Status | Depends On |
|------|--------|------------|
| GIT-015b: Replace synthetic SHA helpers with real go-git plumbing SHAs; write `entries` on Tree; populate `Branch.sha` in `advanceBranchHead` | 🚀 In Progress | ~~GIT-015a~~ ✅ |

**Gaps resolved**: Gap 1 (Tree `entries`), Gap 2 (`advanceBranchHead` sha), Gap 3 (real vs synthetic SHA).

**Scope**:
- `git_impl_fileops.go` — remove `contentSHA()` and `commitSHA()` helpers; compute Blob/Tree/Commit SHAs using `plumbing.MemoryObject` encode+hash (real git binary SHA-1). Serialise child entries as a JSON array `[{name, mode, sha}]` and write to the `entries` property on each Tree entity.
- `git_impl_repo.go` — `advanceBranchHead`: after updating `head_commit_id`, fetch the new Commit entity, read its `sha` property, and write it to `Branch.sha` via `dm.UpdateEntity`.
- `go build ./...` + `go test -race ./...` must pass.

#### GIT-015c — `storer.go` EncodedObjectStorer via DataManager

| Task | Status | Depends On |
|------|--------|------------|
| GIT-015c: Rewrite `arangoStorer` object methods — `SetEncodedObject` / `EncodedObject` / `IterEncodedObjects` / `HasEncodedObject` / `EncodedObjectSize` — via `entitygraph.DataManager` | 📋 Not Started | ~~GIT-015b~~ ✅ |

**Gap resolved**: Gap 5 (object-store half).

**Scope**:
- `storage/arangodb/storer.go` — replace `driver.Database` field with `entitygraph.DataManager`; drop all `gitraw_objects` / `colObjects` references.
- `SetEncodedObject(blob/tree/commit)` → `dm.CreateEntity` on the matching TypeID (`Blob`, `Tree`, `Commit`) with `sha`=hash, raw bytes base64-encoded in `content`/`data`; idempotent on conflict.
- `EncodedObject(type, hash)` → `dm.ListEntities` filtered by TypeID + `sha`; deserialise back to `plumbing.MemoryObject` using the stored binary bytes.
- `HasEncodedObject(hash)` → same list query; return `plumbing.ErrObjectNotFound` on empty result.
- `IterEncodedObjects(type)` → `dm.ListEntities` by TypeID; wrap in `storer.NewEncodedObjectSliceIter`.
- `EncodedObjectSize(hash)` → `dm.ListEntities` filtered by `sha`; read `size` field.
- `AddAlternate` → no-op.
- `go build ./...` + `go test -race ./...` must pass.

#### GIT-015d — `storer.go` ReferenceStorer + internal state storers via DataManager

| Task | Status | Depends On |
|------|--------|------------|
| GIT-015d: Rewrite `arangoStorer` ref and internal-state methods — `SetReference` / `Reference` / `IterReferences` / `RemoveReference` / `CheckAndSetReference` / `Config` / `SetConfig` / `Index` / `SetIndex` / `Shallow` / `SetShallow` / `Module` — via `entitygraph.DataManager` | 📋 Not Started | ~~GIT-015c~~ ✅ |

**Gap resolved**: Gap 5 (refs + internal-state half).

**Scope**:
- `storage/arangodb/storer.go` — drop all `gitraw_refs` / `gitraw_config` / `gitraw_index` / `gitraw_shallow` / `colRefs` / `colConfig` / `colIndex` / `colShallow` references.
- `SetReference(refs/heads/X)` → `dm.ListEntities(TypeBranch, filter name=X)` → `dm.UpdateEntity` with `sha`=hash.
- `SetReference(HEAD symbolic)` → `dm.ListEntities(TypeRepository)` → `dm.UpdateEntity` with `head_ref`=target.
- `Reference(refs/heads/X)` → `dm.ListEntities(TypeBranch, name=X)` → `plumbing.NewHashReference`.
- `Reference(HEAD)` → read `head_ref` from Repository entity → `plumbing.NewSymbolicReference`.
- `IterReferences()` → `dm.ListEntities` TypeBranch + TypeTag → `storer.NewReferenceSliceIter`.
- `RemoveReference` → `dm.DeleteEntity` on Branch or Tag.
- `CheckAndSetReference` → read Branch entity `sha`, compare with `old.Hash()`, update via DataManager; return `storage.ErrReferenceHasChanged` on mismatch.
- `SetConfig` / `Config` → `dm.CreateEntity` / `dm.ListEntities` on `GitInternalState` with `state_type="config"`, `data`=base64 marshalled config.
- `SetIndex` / `Index` → same with `state_type="index"`.
- `SetShallow` / `Shallow` → same with `state_type="shallow"`.
- `Module(name)` → return `newArangoStorer(dm, agencyID+"/module/"+name)`.
- Drop `ensureGitRawCollections`, `colObjects`, `colRefs`, `colConfig`, `colIndex`, `colShallow` constants.
- `go build ./...` + `go test -race ./...` must pass.

#### GIT-015e — Backend refactor + `cmd/main.go` wiring + filesystem removal

| Task | Status | Depends On |
|------|--------|------------|
| GIT-015e: Refactor `arangoBackend` to use `entitygraph.DataManager`; update `cmd/main.go` to single unified backend; remove filesystem backend and its config vars | 📋 Not Started | ~~GIT-015c~~ ✅, ~~GIT-015d~~ ✅ |

**Gap resolved**: Gap 4 (`InitRepo` ownership).

**Scope**:
- `storage/arangodb/backend.go` — replace `driver.Database` field with `entitygraph.DataManager`; `InitRepo` is a no-op (delegates to `gitManager.InitRepo` which already owns entity creation); `OpenStorer` verifies Repository entity exists then returns `newArangoStorer(dm, agencyID)`; `DeleteRepo` / `PurgeRepo` call `dm.DeleteEntity` on the Repository entity. Remove `ensureGitRawCollections` and `NewArangoGitBackend`.
- `storage/arangodb/arangodb.go` — expose `NewArangoStorerBackend(dm entitygraph.DataManager) codevaldgit.Backend` for use in `cmd/main.go`.
- `cmd/main.go` — remove `filesystem` import; construct a single `arangoBackend` from the existing `arangoBackend` DataManager; pass it to `NewGitHTTPHandler`; remove `fsBackend` construction; remove `GIT_REPOS_BASE_PATH` / `GIT_REPOS_ARCHIVE_PATH` references.
- `internal/config/config.go` — remove `ReposBasePath` and `ReposArchivePath` fields and their `Load()` entries.
- `go build ./...` + `go vet ./...` + `go test -race ./...` must all pass.
- Integration smoke test: `InitRepo` via gRPC → `git clone http://localhost:50053/{agencyID}` succeeds.
