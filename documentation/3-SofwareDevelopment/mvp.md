...
| GIT-023b: Add `FetchBranchRequest`, `FetchBranchJob` to `models.go`; `ErrBranchAlreadyFetched`, `ErrBlobContentUnavailable` to `errors.go`; `FetchBranch` + `GetFetchBranchStatus` to `GitManager` interface | � In Progress | GIT-023a |
