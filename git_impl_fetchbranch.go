...
// fetchJobStatus values used as the "status" property of the FetchBranchJob entity.
const (
...
//
// The method returns immediately with a [FetchBranchJob] whose ID can be used
// to poll [GitManager.GetFetchBranchStatus]. Returns [ErrBranchAlreadyFetched]
...
// 4. Transition Branch status → "fetching".
...
//  2. Retrieves bare_clone_path from the Repository entity.
//  3. Opens (or re-clones) the bare repository.
//  4. Deepens the clone to full history for this branch.
//  5. Walks the full commit history and upserts Commit entities (seenSHAs dedup).
//  6. Walks the tip commit tree and upserts Tree + Blob entities (metadata only).
//  7. Advances the branch HEAD pointer.
//  8. Transitions Branch status → "fetched" (or "fetch_failed" on error).
//  9. Transitions job → "completed" (or "failed" on error).
//  10. Publishes cross.git.{agencyID}.branch.fetched.
...
// Retrieve the Repository entity to get bare_clone_path and source_url.
...
// If sourceURL is empty, try the Branch entity's source_url.
if sourceURL == "" {
...
// Open or re-clone the bare repository.
...
// If the bare_clone_path changed (re-clone), persist it on the Repository entity.
...
// Walk the full commit history and upsert Commit entities.
...
// Walk the tip commit tree — Tree + Blob entities (metadata only, no content).
...
// Advance the branch HEAD to the tip Commit entity.
...
// path is empty or the directory no longer exists, it performs a fresh bare
// shallow clone from sourceURL into a new temp directory.
// Returns the opened Repository and the (possibly updated) bare clone path.
...
if sourceURL == "" {
return nil, "", fmt.Errorf("bare_clone_path missing or gone and source_url is empty — cannot re-clone")
}

// Re-clone into a fresh temp directory.
...
// converting the shallow clone to a full clone for that branch.
...
Depth:      0, // unshallow — fetch full history
Force:      true,
}
if sourceURL != "" {
...
// Log but continue — the existing shallow data may still be sufficient.
log.Printf("[deepenClone] FetchContext branch=%q: %v (continuing with existing clone)", branchName, err)
}
return nil
}

// walkCommitsOnly walks all commits reachable from ref and upserts a Commit
// entity for each. seenSHAs deduplicates shared history across multiple fetch
// calls. Trees and blobs are NOT written here; the tip tree is handled separately.
...
// upsertTreeMetadata upserts Tree and Blob entities for a go-git Tree, storing
// only blob metadata (sha, path, name, extension, size) — no content.
// Recursive for subdirectories. ErrEntityAlreadyExists is silently skipped.
...
// upsertBlobMetadata creates a Blob entity carrying only sha, path, name,
// extension, and size — content is intentionally omitted so it can be populated
// lazily by [GitManager.ReadFile] (GIT-023e).
// ErrEntityAlreadyExists is silently skipped.
...
// Read and discard content; we only need to confirm readability and size.
...
"size":       blobObj.Size,
"encoding":   "",
"content":    "", // populated lazily by ReadFile (GIT-023e)
"created_at": now,
},
...
// updateBranchFetchStatus patches the Branch entity's status and optional
...
// findBranchRef resolves a branch name to its reference in the local bare clone.
// It checks refs/heads/<name> first, then refs/remotes/origin/<name>.
